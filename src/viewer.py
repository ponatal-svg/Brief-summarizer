"""Generate the static viewer HTML page."""
...
.ts-chip:hover { color: var(--text); border-color: var(--text-mid); }
...
<header class="header">
  ...
  <h1>Morning Brief<span>/ youtube</span></h1>
...
function md2html(md) {
  if (!md) return '';
  /* Strip legacy [[t=NNs]](url) before generic link processing so only the chip remains */
  md = md.replace(/\\[\\[t=(\\d+)s\\]\\]\\([^)]*\\)/g, function(match, secs) {
    var s = parseInt(secs, 10);
    var mm = Math.floor(s / 60);
    var ss = String(s % 60).padStart(2, '0');
    return '<span class="ts-chip">\\u25b6\\ufe0e ' + mm + ':' + ss + '</span>';
  });
  /* Convert remaining raw [t=NNs] markers (new format) to chips */
  md = md.replace(/\\[t=(\\d+)s\\]/g, function(match, secs) {
    var s = parseInt(secs, 10);
    var mm = Math.floor(s / 60);
    var ss = String(s % 60).padStart(2, '0');
...
async function init() {
  ...
  try {
    var idxResp = await fetch('digest-index.json');
  ...
  if (DIGEST_INDEX.length === 0) {
    document.getElementById('content').innerHTML =
      '<div class="empty">No digests available yet. Run the pipeline first.</div>';
    return;
  }
...
/* ===== DATE LOADING ===== */
// explicit=true when triggered by a user button click — skip auto-fallback
async function loadDate(dateStr, fallback, explicit) {
  currentDate = dateStr;
  allExpanded = false;
  updateExpandBtn();
  stopTTS();
  ...
  try {
    ...
    // Auto-fallback to next older date only on initial page load (not on explicit user clicks)
    var hasContent = md.includes('### ');
    if (!hasContent && !fallback && !explicit) {
      var idx = DIGEST_INDEX.indexOf(dateStr);
      if (idx !== -1 && idx + 1 < DIGEST_INDEX.length) {
        loadDate(DIGEST_INDEX[idx + 1], true, false);
        return;
      }
    }
...
async function toggleSummary(id) {
  ...
  if (!body.dataset.loaded) {
    ...
    try {
      ...
      /* Strip markdown syntax, URLs and timestamp markers from TTS text */
      var plainBody = ttsBody
        .replace(/\\[\\[t=\\d+s\\]\\]\\([^)]*\\)/g, '') // strip [[t=NNs]](url) old format
        .replace(/\\[t=\\d+s\\]/g, '')                   // strip [t=NNs] timestamp markers
        .replace(/https?:\\/\\/[^\\s)]+/g, '')
        .replace(/\\[([^\\]]+)\\]\\([^)]+\\)/g, '$1')
        .replace(/[#*\\[\\]()>]/g, '')
        .replace(/\\n+/g, ' ')
...
function extractTTSBody(summaryMd) {
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    if (/^#{2,3}\\s+The Hook$/.test(trimmed)) { currentSection = 'hook'; continue; }
    if (/^#{2,3}\\s+Key Findings:?$/.test(trimmed)) { currentSection = 'findings'; result += 'Key findings. '; continue; }
    if (/^#{2,3}\\s+The So What\\??$/.test(trimmed)) { currentSection = 'sowhat'; result += 'So what. '; continue; }
...
function parseSummaryMd(md, cardId) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var trimmed = lines[i].trim();
    if (/^#{2,3}\\s+The Hook$/.test(trimmed)) { currentSection = 'hook'; continue; }
    if (/^#{2,3}\\s+Key Findings:?$/.test(trimmed)) { currentSection = 'findings'; continue; }
    if (/^#{2,3}\\s+The So What\\??$/.test(trimmed)) { currentSection = 'sowhat'; continue; }
    ...
    // Also accept lines starting with - as bullet points in findings
    if (currentSection === 'findings' && trimmed.startsWith('-') && !trimmed.startsWith('--')) {
      findings.push(trimmed.replace(/^-\\s*/, '').replace(/^\\*\\*(.+?):\\*\\*/, '<strong>$1:</strong>'));
    }
  ...
  /* Extract video ID for timestamp links (YouTube only) */
  var videoId = '';
  var vidIdMatch = watchUrl.match(/[?&]v=([A-Za-z0-9_-]+)/);
  if (vidIdMatch) videoId = vidIdMatch[1];

  /* Render Key Findings — convert [t=NNs] markers to clickable timestamp chips */
  if (findings.length) {
    html += '<div class="section-label">Key Findings</div>';
    for (var j = 0; j < findings.length; j++) {
      var num = String(j + 1).padStart(2, '0');
      var findingHtml = findings[j];
      /* Strip legacy [[t=NNs]](url) markdown links — generated before viewer handled chips.
         Both patterns are collapsed to a raw [t=NNs] marker first, then converted below. */
      findingHtml = findingHtml.replace(/\\[\\[t=(\\d+)s\\]\\]\\([^)]*\\)/g, '[t=$1s]');
      /* Replace [t=NNs] with a clickable ▶ MM:SS chip */
      findingHtml = findingHtml.replace(/\\[t=(\\d+)s\\]/g, function(match, secs) {
        var s = parseInt(secs, 10);
        var mm = Math.floor(s / 60);
        var ss = String(s % 60).padStart(2, '0');
        var label = '\\u25b6\\ufe0e ' + mm + ':' + ss;
        if (videoId) {
          return '<a href="https://www.youtube.com/watch?v=' + videoId + '&t=' + s + '" target="_blank" class="ts-chip">' + label + '</a>';
        }
        return '<span class="ts-chip">' + label + '</span>';
      });
      html += '<div class="finding"><span class="finding-num">' + num + '</span><span>' + findingHtml + '</span></div>';
    }
  }
...
/* ===== FILTER ===== */
async function applyFilter(cat) {
  currentFilter = cat;
  ...
  document.querySelectorAll('.card').forEach(function(c) {
    ...
    if (show) {
      count++;
      // If "expand all" is active, newly-visible cards that aren't open yet need expanding
      if (allExpanded && !c.classList.contains('open')) {
        toExpand.push(parseInt(c.id.replace('card-', '')));
      }
    }
  });
  var countEl = document.getElementById('visibleCount');
  if (countEl) countEl.textContent = count;
  // Expand cards that were hidden during the previous toggleExpandAll call
  for (var i = 0; i < toExpand.length; i++) {
    await toggleSummary(toExpand[i]);
  }
}
...
def generate_viewer(config: Config, output_dir: Path) -> None:
    ...
    # Write youtube.html (YouTube detail page; index.html is the hub)
    index_path = output_dir / "youtube.html"
    index_path.write_text(VIEWER_HTML, encoding="utf-8")

    # Write categories.json (name -> color mapping)
    categories = {c.name: c.color for c in config.categories}
